...
void my_handler(int signum){
	...
	//	while(running_queue->size>0 && firstswap == 0){
		...
		node* wait_node = waiting_queue->front;
		while(waiting_node!=NULL){
			tcb* checkthread = waiting_node->thread;
			if(checkthread->state==waitjoin){ //joins
				node* done_node = done_queue->front;
				while(done_node!=NULL){
					if(done_node->thread->tid==wait_node->thread->waitid){ //finds successs
						//can get return val from done_node
						//need to set return_vall they give me
						
					}
					done_node= done_node->next;
				}
			}
			else if (checkthread->state==waitmutex){ //mutexes
			
			}
			waiting_node = waiting_node->front;
		}
...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	/*
	Plan B: If this doesn't work, leave thread in join
	and use while loop to make it keep searching the done queue.
	No more waiting queue, join thread just sits in the running queue and scheduler
	switches betweeen threads while it waits.
	*/
	if(done_queue->size==0){
		...
		while(search!=NULL){
			if(search->thread->tid==thread){ //match found
				if(value_ptr!=NULL){
	...
	node* wait = (node*) malloc(sizeof(node));
	running_thread->state = waitjoin;
	running_thread->waitid = thread;
	wait->thread=running_thread;
	enqueue_other(wait,waiting_queue);
	my_pthread_yield();
};
